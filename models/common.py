...
# 改进 BiFPN # 结合BiFPN 设置可学习参数 学习不同分支的权重 只有yolov5s可用
# 两个分支add操作
...
class GhostConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):  # ch_in, ch_out, kernel, stride, groups
        ...
        self.cv1 = Conv(c1, c_, k, s, None, g, act) # 先进行一半卷积 减少计算量 第一步，使用少量卷积核进行卷积操作（比如正常用64个，这里就用32个，从而减少一半计算量）
        self.cv2 = Conv(c_, c_, 5, 1, None, c_, act)# 再进行逐特征图卷积 第二步，使用3×3或5×5的卷积核进行逐通道卷积操作（Cheap operations）
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):  # ch_in, ch_out, kernel, stride
        ...
        self.conv = nn.Sequential(
            ...
            DWConv(c_, c_, k, s, act=False) if s == 2 else nn.Identity(),  # dw 当stride=2时 才开启 stride=1，不进行下采样时，直接进行两个Ghost卷积操作 stride=2，进行下采样时，多出来一个步长为2的深度卷积操作
